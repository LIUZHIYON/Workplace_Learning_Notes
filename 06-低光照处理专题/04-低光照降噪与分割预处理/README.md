...
> 🧰 **配套 GUI 工具:** `median_clahe_tool.py` — 双击 `启动工具.bat` 即可运行。支持中文路径图片加载、参数实时调节、保存增强结果。
...
项目附带了一个 PyQt5 图形界面工具 median_clahe_tool.py，支持：
- 📂 打开图片（支持中文文件名）
- 🎛️ 实时调节中值滤波核大小、CLAHE clip 和分块参数
- ◀▶ 左右分屏对比原图和处理结果
- 💾 保存增强结果（支持中文路径）

**启动方式：** 双击 启动工具.bat 或运行：
`ash
python median_clahe_tool.py
`
